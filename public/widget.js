...
(function () {
  ...
  function flip() {
    ...
    tog(w, "opacity-0", !open); tog(w, "scale-95", !open); tog(w, "pointer-events-none", !open);
    tog(w, "opacity-100", open); tog(w, "scale-100", open);
    tog(o, "opacity-0", open); tog(o, "scale-50", open);
    tog(x, "opacity-0", !open); tog(x, "scale-50", !open);
    tog(x, "opacity-100", open); tog(x, "scale-100", open);
  ...
  async function send(e) {
    ...
    try {
      ...
      if (!r.ok) throw 0;
      const rd = r.body.getReader(); const dc = new TextDecoder(); let t = "", idx = null;
      while (1) {
  ...
  async function load() {
    try {
      ...
      if (r.ok) { const d = await r.json(); if (d.messages?.length) { msgs = d.messages; draw(); } }
    } catch {}
  }
